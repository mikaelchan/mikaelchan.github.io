+++
date = '2025-01-07T13:03:30+08:00'
...
本文在A100(SXM4-80GB)上做了四组实验：朴素全局内存方法、共享内存分块方法、利用 Padding（填充）优化的共享内存方法、利用 swizzling优化共享内存和对角线访问DRAM的方法，来学习CUDA上的优化技巧。
> [!INFO]矩阵为4096x4096xfp32, 吞吐从218.67GB/s 提升至1796.98GB/s。
...
### 1.1 内存层次结构与延迟隐藏
GPU 计算的根本挑战在于“内存墙 (Memory Wall)”。虽然我是用的 A100 的计算吞吐量标注 19.5TFLOPS（fp32 cuda core），但内存带宽仅在 2 TB/s。对于像矩阵转置这样的算子，它执行纯粹的数据排列\(A_{ij}​→B_{ji}​\) 而没有数据重用或算术规约，其运算强度正好是每访问 1 字节传输 1 字节（假设完美缓存），甚至更差。
本文会涉及到3个不同层级的内存：
...
### 1.2 加载/存储单元（LSU）与合并访存（Coalescing）
...
+ 合并访问: 如果 Warp 发出的地址落在连续且对齐的区域内（例如，由 4 个 32 字节扇区组成的单个 128 字节缓存行），LSU 可以用最少数量的事务（通常是 4 个 32 字节事务）为整个 Warp 服务。
> [!INFO]这与锁步执行也有直接关系，但讨论这个还要考虑分支发散，为了省事略过。

+ 非合并访问: 如果地址是分散的（跨步的），LSU 必须为请求的每个不同扇区发出单独的事务。这种现象称为“扇区发散 (Sector Divergence)”，它会倍增内存总线上的流量并可能使 LSU 的内部队列饱和 。

### 1.3 加载/存储队列（LSQ）与停顿机制（stall）
...
+ 线程 0 写入地址 A。
+ 线程 1 写入地址 A+4096 floats=A+16384 字节。
+ 线程 2 写入地址 A+32768 字节。
