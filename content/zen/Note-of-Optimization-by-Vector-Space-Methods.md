...
证明定义在$C[0, 1]$ {{<sidenote>}} 范数定义如下：$$\\|x\\|=\max_{t \in [0,1]}{|x(t)|}$$ {{</sidenote>}} 中的单位球面不是紧集。
...
**投影定理**：
设$H$是希尔伯特空间，$M$是$H$的**闭子空间**。对任一矢量$x \in H$，都有唯一的矢量$m_0 \in M$，使得$\\|x-m_0\\|\le\\|x-m\\|$，并且$m_0 \in M$是唯一的极小化矢量的充要条件是$m_0 \perp M$。{{<sidenote>}}这里闭子空间保证存在性。![投影定理](/images/projection.svg){{</sidenote>}}
...
\end{pmatrix}
$$
称为$y_1, y_2, \dots, y_n$的格拉姆矩阵。{{<sidenote>}}$G(y_1, y_2, \dots, y_n)$是半正定的。{{</sidenote>}}
...
$$
1.计算分子（内积）{{<sidenote>}}奇函数在对称区间的积分为 0{{</sidenote>}}：
$$
...
$$
1.计算第一个投影分量（对 $u_0$）：$$\langle x_2, u_0 \rangle = \int_{-1}^{1} t^2 \cdot 1 \, dt = \frac{2}{3}$$系数为：$\frac{2/3}{2} = \frac{1}{3}$
...
+ 单位球面上的上确界{{<sidenote>}}仅当 $X \neq \{0\}$ 时适用{{</sidenote>}}：$$\\|f\\| = \sup_{\\|x\\| = 1} |f(x)|$$
